...
export async function GET(){
 ...
 const titles=(j.rows||[]).slice(1).filter(r=>r[0]).map(r=>({id:r[12],title:r[0],serviceHint:r[1]||'',type:r[2]||'',year:r[3]||'',status:r[7]||'Want to Watch',notes:r[9]||'',availability:r[16]||'needs_review',matchStatus:r[17]||'needs_review',checkedAt:r[20]||'',sourceUrl:r[19]||'',...((artwork as Record<string,unknown>)[String(r[12])]||{poster:null,overview:'',genres:[]}),firstReleaseDate:r[22]||dates[String(r[12])]?.firstReleaseDate||'',nextReleaseDate:r[23]||dates[String(r[12])]?.nextReleaseDate||'',lastReleaseDate:r[24]||dates[String(r[12])]?.lastReleaseDate||'',providers:av.slice(1).filter(p=>p[0]===r[12]&&p[3]==='US'&&['flatrate','free','ads'].includes(String(p[6]))).map(p=>({provider_id:p[4],provider_name:p[5],access_type:p[6]}))}));
